...
__call__() # Эта функция будет вызываться и испольняться каждый раз. Если это объект класса. (show = Show()()) Каждая функция обладает этим методом. Если данный метод существует то значит функцию можно выполнить.
__getattr__(self, name) # obj.name  Данный метод срабатывает если объект вообще не имеет атрибута с именем name. name это сам атрибут (obj.name => name)
__setattr__(self, name, value) # obj.name = value. Вызывается всегда не зависимо есть такой атрибут или нет
